...
**Через UI (вкладка «Пресеты»):**
- Перетащите .wav, .mp3, .ogg, .flac или .m4a в дропзону (или нажмите — выберите файл)
- Файл автоматически конвертируется в WAV и сохраняется в `voices/`
- Голос сразу появляется в выпадашке «Голос» на вкладке «Генерация»
- Можно удалить пресет кнопкой ✕

**Через REST API:**
```bash
# Загрузить
curl -s -X POST http://localhost:7861/api/upload_preset \
  -F "file=@голос.mp3" \
  -F "name=Алексей"

# Список
curl -s http://localhost:7861/api/voices

# Удалить
curl -s -X DELETE http://localhost:7861/api/voices/Алексей
```
