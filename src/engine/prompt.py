...
Ported from sgl-project/sglang-omni ``higgs_tts/text_tokenizer.py`` (Apache-2.0).

Assembles, depending on ref-audio + ref-text presence:

- voice-clone + transcript: ``<|tts|> <|ref_text|> tok(ref) <|ref_audio|> [-100]xN <|text|> tok(text) <|audio|>``
- voice-clone, no transcript: ``<|tts|> <|ref_audio|> [-100]xN <|text|> tok(text) <|audio|>``
- zero-shot:                  ``<|tts|> <|text|> tok(text) <|audio|>``

``<|tts|>`` selects task mode (vs ASR). ``-100`` placeholders are replaced at runtime
by the fused embedding of the *delayed* reference codes; ``num_ref_tokens`` MUST equal
the delayed ref-code row count (``T + num_codebooks - 1``).
"""
