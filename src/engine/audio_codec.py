"""Thin facade over the vendored Higgs Audio V2 tokenizer (the v3 codec).

Ported from sgl-project/sglang-omni ``higgs_tts/audio_codec.py`` (Apache-2.0); the
only change is the import path of the vendored tokenizer.

The codec weights are bundled inside the Higgs TTS checkpoint under the prefix
``tied.embedding.modality_embeddings.0.model.*`` (acoustic_encoder, acoustic_decoder,
quantizer, semantic_model, etc.). :meth:`HiggsAudioCodec.from_pretrained` extracts
those keys directly from a checkpoint dir's ``model.safetensors`` so a single
checkpoint serves both the AR engine and the codec. (Our converter writes those
keys, prefix intact, into ``models/codec/model.safetensors``.)
"""
...
from ._vendored.higgs_audio_v2_tokenizer_hf import (
...
def _load_codec_state_dict(tts_ckpt_dir: str) -> dict[str, torch.Tensor]:
    ...
    the checkpoint's shards into a state dict keyed by the codec's own parameter
...
class HiggsAudioCodec:
    ...
    def from_pretrained(
        ...
        *,
        ...
        """Load codec from a checkpoint dir (local) or HF repo id.
    ...
    def encode_reference(
        ...
        *,
        ...
        ``[-1, 1]``. Resamples to 24 kHz; clips < 1 s are zero-padded.
        """
